...
import { Project, STAGE_DISPLAY_NAMES, Priority } from '@antigravity/shared';
      ...
      {activeTab === 'team' && (
        <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
          ...
          {(!project.members || project.members.length === 0) ? (
            <p className="text-xs text-slate-400 italic">No employees assigned to this project team yet.</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {project.members.map((m: any) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-slate-900 truncate">{m.user?.name}</p>
                    <p className="text-[11px] text-slate-500 truncate">{m.user?.department || m.user?.email}</p>
                  </div>
                </div>
              ))}
            </div>
