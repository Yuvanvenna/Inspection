...
export const ManagerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    fetchDashboardData();
    // Load clients for the create project modal
    ClientService.getClients().then((cList) => {
      setClients(cList);
      if (cList.length > 0) {
        setNewProjectForm((prev) => ({ ...prev, client_id: cList[0].id }));
      }
    });
  }, []);
  ...
  const handleQuickCreateProject = async (e: React.FormEvent) => {
    ...
    let targetClientId = newProjectForm.client_id;
    if (!targetClientId) {
      if (clients.length > 0) {
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl border border-slate-100 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleQuickCreateProject} noValidate className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. NextGen Autonomous Inspection Rover"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    ...
                    {clients.length === 0 && (
                      <option value="10000000-0000-0000-0000-000000000001">Apex Aerospace (Apex Dynamics Inc.)</option>
                    )}
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-700">Target Deadline *</label>
                  <input
                    type="date"
                    required
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700">Project Scope & Description</label>
                <textarea
                  rows={3}
                  value={newProjectForm.description}
                  onChange={(e) => setNewProjectForm({ ...newProjectForm, description: e.target.value })}
                  placeholder="Engineering deliverables, specifications, and scope summary..."
                  className="mt-1 w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:outline-none"
                ></textarea>
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold shadow-sm disabled:opacity-50"
                >
